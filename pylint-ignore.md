...
# Overview
 ...
 - [E1101: no-member (20x)](#e1101-no-member)
...
## File src/lib3to6/fixers_unpacking_generalization.py - Line 128 - E1101 (no-member)
...
```
  106: def _expand_stararg_g12n(node: ast.AST) -> ast.expr:
  ...
  126:         tail_list = operands[-1]
  127:         assert isinstance(tail_list, ast.List)
> 128:         tail_elts = tail_list.elts
  129:
  130:         if not isinstance(elt, ast.Starred):
```


## File src/lib3to6/fixers_unpacking_generalization.py - Line 157 - E1101 (no-member)
...
> 157:     if len(tail_list.elts) == 0:
  158:         operands = operands[:-1]
  159:
```
...
## File src/lib3to6/fixers_unpacking_generalization.py - Line 182 - R0912 (too-many-branches)
...
```
  178: class UnpackingGeneralizationsFixer(fb.FixerBase):
  ...
  180:     version_info = common.VersionInfo(apply_since="2.0", apply_until="3.4")
  181:
> 182:     def expand_starstararg_g12n(self, node: ast.expr) -> ast.expr:
  183:         chain_values: typ.List[ast.expr] = []
  184:         chain_val   : ast.expr
```
